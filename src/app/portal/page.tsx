  ...
  return (
    <div>
      ...
      <div className="card mb-4 p-4 flex flex-wrap gap-2 items-center">
        ...
        <code className="rounded bg-ink-100 px-2 py-1 text-xs dark:bg-white/10">{process.env.NEXT_PUBLIC_BASE_URL ?? ""}/portal/{u.id.slice(0, 8)}</code>
            ...
            <tbody>
              {invoices.map((inv) => (
                <tr key={inv.id}>
                  ...
                  <td className="text-ink-500">{(inv.buyerSnapshot ? JSON.parse(inv.buyerSnapshot as string)?.companyName ?? JSON.parse(inv.buyerSnapshot as string)?.name : "—") as string}</td>
