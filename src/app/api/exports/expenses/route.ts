...
export async function GET() {
  ...
  const header = ["date", "description", "supplier", "category", "amountHT", "total", "currency", "paymentMethod", "reference", "billable"].join(";");
  const lines = rows.map((e) => [
    new Date(e.date).toISOString().slice(0, 10),
    e.description, e.supplier ?? "", e.category ?? "",
    (e.amountHTMinor / 100).toFixed(2), (e.totalMinor / 100).toFixed(2), e.currency,
