...
export default async function PaymentLinksPage() {
  ...
  if (!u) redirect("/login");
  const invoices = await prisma.invoice.findMany({ where: { ownerId: u.id, status: "ISSUED" }, select: { id: true, invoiceNumber: true, totalTTC: true, currency: true }, orderBy: { createdAt: "desc" }, take: 100 });
  const links = await prisma.paymentLink.findMany({ where: { ownerId: u.id }, include: { invoice: { select: { invoiceNumber: true } } }, orderBy: { createdAt: "desc" }, take: 100 });
  return (
    <div>
