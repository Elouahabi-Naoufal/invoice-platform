...
export default async function LettragePage() {
  ...
  if (!u) redirect("/login");
  const avoirs = await prisma.invoice.findMany({ where: { ownerId: u.id, docType: "AVOIR" }, select: { id: true, invoiceNumber: true, totalTTC: true, currency: true }, orderBy: { createdAt: "desc" }, take: 100 });
  const invoices = await prisma.invoice.findMany({ where: { ownerId: u.id, status: "ISSUED" }, select: { id: true, invoiceNumber: true }, orderBy: { createdAt: "desc" }, take: 100 });
  const rows = await prisma.avoirInvoice.findMany({ where: { ownerId: u.id }, include: { avoir: { select: { invoiceNumber: true } }, invoice: { select: { invoiceNumber: true } } }, orderBy: { createdAt: "desc" }, take: 100 });
  return (
    <div>
