...
export default async function LettragePage() {
  ...
  const avoirs = await safeFindMany(() => prisma.invoice.findMany({ where: { ownerId, docType: "AVOIR" }, select: { id: true, invoiceNumber: true, totalTTC: true, currency: true }, orderBy: { createdAt: "desc" }, take: 100 }), []);
  const invoices = await safeFindMany(() => prisma.invoice.findMany({ where: { ownerId, status: "ISSUED" }, select: { id: true, invoiceNumber: true }, orderBy: { createdAt: "desc" }, take: 100 }), []);
  const rows = await safeFindMany(() => prisma.avoirInvoice.findMany({ where: { ownerId }, include: { avoir: { select: { invoiceNumber: true } }, invoice: { select: { invoiceNumber: true } } }, orderBy: { createdAt: "desc" }, take: 100 }), []);
  return (
    <div>
