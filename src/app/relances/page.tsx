...
export default async function RelancesPage() {
  ...
  if (!u) redirect("/login");
  const overdue = await prisma.invoice.findMany({ where: { ownerId: u.id, status: "ISSUED", dueDate: { lt: new Date() } }, orderBy: { dueDate: "asc" }, take: 100 });
  const reminders = await prisma.reminder.findMany({ where: { ownerId: u.id }, include: { invoice: { select: { invoiceNumber: true, totalTTC: true, currency: true } } }, orderBy: { scheduledAt: "desc" }, take: 100 });
  const issued = await prisma.invoice.findMany({ where: { ownerId: u.id, status: "ISSUED" }, select: { id: true, invoiceNumber: true, dueDate: true }, orderBy: { dueDate: "asc" }, take: 100 });
  return (
    <div>
