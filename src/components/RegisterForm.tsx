...
export default function RegisterForm() {
  ...
  async function submit(f: FormData) {
    ...
    try {
      await registerTenant({
        ...
        requestedSlug: String(f.get("requestedSlug") ?? ""),
      });
      setDone(true);
  ...
  return (
    <form action={submit} className="grid gap-4">
      ...
      <div><label className="label">Requested slug *</label><input name="requestedSlug" required pattern="[a-z0-9-]+" className="input" placeholder="my-company" /></div>
